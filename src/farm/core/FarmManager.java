 ...
 * <p>
 * <b>The FarmManager file is provided for you, please move it from outside the
 * {@code src} into {@code src/farm/core} before beginning work on it.</b>
 * <br> The project won't compile if it is moved into place before Farm is ready.
 * <p>
 * Whilst a lot of the Manager has been provided to you there are four methods you need
 * to implement to make it work.
 * <br>
 * Method stubs have been provided to ensure the code compiles and these
 * methods have been made protected.
 * <p>
 * Whilst the majority of FarmManager is implemented in stage2, the concept of quantities
 * isn't introduced till stage3, at which point the FarmManager will need to be extended.
 * <p>
 ...
 * {@link  FarmManager#addToInventory(String, int)}, {@link FarmManager#createCustomer()},
...
public class FarmManager {
    ...
    protected void addToInventory(String productName) {
        ...
        if (Objects.equals(productName, Barcode.EGG.getDisplayName())) {
            this.farm.stockProduct(Barcode.EGG, Quality.REGULAR);
            successful = true;
        } else if (Objects.equals(productName, Barcode.JAM.getDisplayName())) {
            this.farm.stockProduct(Barcode.JAM, Quality.REGULAR);
            successful = true;
        } else if (Objects.equals(productName, Barcode.MILK.getDisplayName())) {
            this.farm.stockProduct(Barcode.MILK, Quality.REGULAR);
            successful = true;
        } else if (Objects.equals(productName, Barcode.WOOL.getDisplayName())) {
            this.farm.stockProduct(Barcode.WOOL, Quality.REGULAR);
            successful = true;
        } else {
            this.shop.displayProductAddFailed("Provided product name is invalid");
        }

        if (successful) {
            this.shop.displayProductAddSuccess();
        }
    }
    ...
    protected void addToInventory(String productName, int quantity) {
        ...
        if (!this.enableFancy) {
            this.shop.displayProductAddFailed("Current inventory is not fancy enough"
                + " to handle adding more than one product.");
        }

        boolean successful = false;
        try {
            if (Objects.equals(productName, Barcode.EGG.getDisplayName())) {
                this.farm.stockProduct(Barcode.EGG, Quality.REGULAR, quantity);
                successful = true;
            } else if (Objects.equals(productName, Barcode.JAM.getDisplayName())) {
                this.farm.stockProduct(Barcode.JAM, Quality.REGULAR, quantity);
                successful = true;
            } else if (Objects.equals(productName, Barcode.MILK.getDisplayName())) {
                this.farm.stockProduct(Barcode.MILK, Quality.REGULAR, quantity);
                successful = true;
            } else if (Objects.equals(productName, Barcode.WOOL.getDisplayName())) {
                this.farm.stockProduct(Barcode.WOOL, Quality.REGULAR, quantity);
                successful = true;
            } else {
                this.shop.displayProductAddFailed("Provided product name is invalid");
            }
            ...
            this.shop.displayProductAddFailed("Quantity must be at least 1.");
        }
    }


    /**
     * Prompt the user to create a new customer and then save it to the farms address book
     * for later usage.
     */
    protected void createCustomer() {
        try {
            String name = shop.promptForCustomerName();
            int number = shop.promptForCustomerNumber();
            String address = shop.promptForCustomerAddress();

            farm.saveCustomer(new Customer(name, number, address));
    ...
    protected void initiateTransaction(String transactionType) {
        try {
            ...
            Customer customer = farm.getCustomer(name, number);

            if (Objects.equals(transactionType, "-s") || Objects.equals(transactionType,
                    "-specialsale")) {
                Map<Barcode, Integer> discounts = this.getDiscounts();
                shop.displayTransactionStart();
                farm.startTransaction(new SpecialSaleTransaction(customer, discounts));
            } else if (Objects.equals(transactionType, "-c") || Objects.equals(transactionType,
                    "-categorised")) {
                shop.displayTransactionStart();
                farm.startTransaction(new CategorisedTransaction(customer));
            } else {
                shop.displayTransactionStart();
                farm.startTransaction(new Transaction(customer));
            }
